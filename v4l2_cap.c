...
extern int process_frame_cuda(const unsigned char *yuyv, unsigned char *rgba,
...
int init_framebuffer(unsigned short **fbPtr, int *size)
{
    ...
    // ioctl은 디바이스와 소통하기 위한 시스템 호출로 하드웨어 장치의 설정과
    // 상태를 제어할 때 사용. 파일디스크립터를 통해 장치에 명령을 전달하고 특정
    // 명령에 대한 처리를 요청 아래의 함수는 프레임버퍼 장치에 FBIOGET_VSCREENINFO
    // 명령을 보내 화면 정보를 가져오라는 요청을 하는 것 성공적으로 호출되면
    // 프레임버퍼의 화면 설정 정보가 vinfo라는 fb_var_screeninfo 구조체에 저장됨
    ...
    *size = vinfo.yres_virtual * vinfo.xres_virtual * vinfo.bits_per_pixel / 8;
    // mmap을 통해 메모리를 매핑하여 물리 메모리와 프레임 버퍼를 연결, 매핑된
    // 메모리 주소가 반환되며, 이주소를 사용해 프레임버퍼에 데이터를 직접 쓸 수
    // 있다.
...
static int init_v4l2(int *fd, struct buffer *buffers)
{
    ...
    // v4l2에서 비디오 프레임 데이터를 처리하기 위해 사용하는 버퍼에 대한 정보를
    // 담고 있다. ioctl함수에서 VIDIOC_QUERYBUF, VIDIOC_QBUF, VIDIOC_DQBUF와 같은
    // 명령어와 함께 사용된다. 이 버퍼는 비디오 데이터를 큐에 넣고 큐에서 빼는
    // 작업을 위해 사용됨.
    ...
    *fd = open(VIDEO_DEV, O_RDWR);
    ...
    // 버퍼의 메모리를 설정함. 아래 처럼 설정하게 되면 MMAP을 통해 커널 메모리를
    // 사용자 공간으로 매핑하는 것
    ...
    for (i = 0; i < BUFFER_COUNT; i++)
    {
        ...
        // 드라이버에서 비디오 장치의 버퍼에 대한 정보를 조회하는 명령 ->
        // VIDIOC_REQBUFS로 버퍼를 요청한 후, 각 버퍼에 대해 VIDIOC_QUERYBUF를
        // 사용하여 해당 버퍼의 상태를 확인하고 메모리를 매핑하는데 필요한 정보를
        // 가져옴
        ...
        // 카메라로부터 비디오 프레임을 캡쳐하면 buf에 저장되는데, 그것을 유저
        // 영역에서 접근할 수 없기 때문에 mmap을 통해서 buffers라는 구조체로 buf에
        // 접근할 수 있도록 설정해주는 것!
    ...
    // 비디오 스트리밍을 시작하는 시스템 콜 -> 이 시스템 콜을 호출하면 큐에있는
    // 버퍼에 비디오 프레임이 저장되게 되는데 이것을 사용하기 위해 나중에 디큐를
    // 해주는 것!
...
int main(int argc, char **argv)
{
    ...
    while (cond)
    {
        ...
        // init_v4l2함수에서 캡쳐한 비디오 프레임이 mmap(buffers[buf.index]).start에
        // 저장되어있는데 이것을 인자로 넣어준다.
        if (process_frame_cuda(buffers[buf.index].start, rgbBuffer, WIDTH, HEIGHT) <
            0)
        {
