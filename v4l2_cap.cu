...
// 프레임버퍼를 설정하는 함수
int init_framebuffer(unsigned char **fbPtr, int *size) 
{
    //frame buffer 장치 열기
    ...
    *fbPtr = (unsigned char *)mmap(0, *size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (*fbPtr == MAP_FAILED) {
...
int main(int argc, char** argv) 
{
    ...
    // 영상을 저장할 메모리 할당
    rgbBuffer = (unsigned char *)malloc(fbSize);
    if (!rgbBuffer) {
	...
	// STREAMON 명령으로 캡쳐한 비디오 프레임을 디큐한다. 
        ...
        if (process_frame_cuda((const unsigned char *)buffers[buf.index].start, rgbBuffer, WIDTH, HEIGHT) < 0) {
